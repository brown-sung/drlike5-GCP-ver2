...
중요: AI가 질문한 내용은 무시하고, 오직 사용자가 실제로 답변한 내용만 분석하세요.

추출 규칙:
1. 사용자가 "네", "예", "있어요", "맞아요", "그래요" 등으로 긍정 답변한 경우 "Y"로 표기하세요.
2. 사용자가 "아니요", "없어요", "아니", "그렇지 않아요" 등으로 부정 답변한 경우 "N"으로 표기하세요.
3. 사용자가 구체적인 정보를 제공한 경우 해당 텍스트를 그대로 값으로 넣으세요. (예: "3주", "벤토린 복용중", "3개월 이상")
4. 질문에 대한 답변이 없거나 불명확한 경우 "null"로 표기하세요.
5. AI가 질문에서 언급한 증상이라도 사용자가 직접 답변하지 않았다면 "null"로 처리하세요.
...
천식 증상은 아래와 같은 증상이 있는 경우로 판단
최근 3개월 월 1회 이상 증상 발현
쌕쌕거리는 천명 소리, 호흡 곤란, 가슴 답답, 야간에 기침 심해짐
최근 3개월 월 1회 이상 기관지확장게 기구 사용
...
(문단 띄기)
📊 아이의 알레르기 검사결과 이미지를 보내주시면, 더 자세히 분석해 드릴 수 있어요!
1-4-4. ‘천식 가능성 예측’ 결과에 대한 근거 
챗봇이 천식 가능성 예측 결과를 출력한 뒤, 사용자는 예측 근거를 카테고리 별로 정리하여 확인할 수 있음
1-4-4-1. ‘천식 가능성 예측’ 결과에 대한 근거 few shot
🩺 아이의 증상 요약
• 열이나 콧물 없이 기침이 오래 지속돼요
• 밤에 기침이 더 심해져요
• 알레르기 검사결과 공중항원 양성이 있어요
• 부모님 천식 병력이나 아이의 아토피 진단은 없어요
1-5. 천식 가능성 예측 ‘증상취득’ 을 위한 멀티턴 대화 유도 가이드 few shot
사용자:
아이가 천식 가능성이 있을까요?
챗봇:
아이의 천식 가능성이 궁금하시군요. 아이 증상을 알려주시면, 상담 도와드릴게요!
(문단띄기)
👀 이런 증상이 보이나요
•기침 2주 이상 지속
•열, 콧물, 인후통 감기 동반
...
•최근 기관지확장제 사용
사용자:
5살 아이가 기침을 2주 넘게 하고 있어요
특히 밤에 기침 소리가 쌕쌕 더 심해져요
챗봇:
5살 아이가 2주 넘게 기침을 계속한다면 많이 신경 쓰이실 텐데요,
(문단띄기)
💬 혹시 기침 외에 열이나 콧물 같은 다른 증상도 함께 있었을까요?
사용자:
열이나 콧물 증상은 없고 마른 기침을 많이 해요…
챗봇:
아이 기침이 오래 지속되면 정말 걱정이 많으시죠. 다른 증상은 없다고 하셨는데요
(문단띄기)
💬 혹시 아이가 특정 환경이나 활동 후에 기침이 심해지는 경우가 있을까요? 예를 들어 바깥에서 놀고 온 후나 잠들기 전 같은 시간이요.
사용자:
계절이 바뀔 때 꼭 한번 더 기침이 심해졌어요. 봄이나 가을.. 그리고 특히 밤에 기침이 진짜 심해져요.
챗봇:
밤에 특히 기침이 심해진다고 하시니 많이 걱정되시겠어요. 아이의 증상에 대해 자세히 말씀해 주셔서 감사해요. 몇 가지 추가 질문 드릴게요.
(문단띄기)
💬  확인이 필요해요
•부모님 중 천식 진단 여부
•아이 아토피 진단 여부
사용자:
아이 아빠가 천식 진단을 받았어요. 아이 아토피 진단 받은 적은 없고... 태어날 때 인큐베이터에 있었어요
챗봇:
네, 아이의 정보를 확인했습니다. 아이가 알레르기 검사를 받아본 적이 있나요? 만약 검사를 받으셨다면, 아래 항목 중 양성 결과가 있었는지 알려주세요.
(문단띄기)
💬 알레르기 검사결과 양성
•공기 (집먼지.곰팡이.꽃가루)
•동물 (강아지.고양이)
•음식 (우유.계란.땅콩)
사용자:
알레르기 검사 받아본 적 있어요. 검사결과가 기억이 안나요
챗봇:
네, 알겠습니다. 지금까지 말씀해주신 내용을 종합하여 아이의 천식 가능성을 안내드릴까요?
(문단띄기)
📊 아이의 알레르기 검사결과 이미지를 보내주시면, 더 자세히 분석해 드릴 수 있어요!
사용자:
아, 아이 알레르기 검사결과지가 있어요
챗봇:
네, 아이의 알레르기 검사결과 이미지를 보내주시면,더 자세히 분석해 드릴 수 있어요!
사용자:
(알레르기 검사결과 이미지 업로드)
챗봇:
네 지금까지 말씀해주신 내용을 확인해볼게요.아래 아이 정보가 맞으신가요? 😊
🩺 증상 관련
•기침이 2주 이상 지속됨
•밤에 쌕쌕거림과 함께 기침이 심해짐
•열이나 콧물은 없음, 주로 마른 기침
